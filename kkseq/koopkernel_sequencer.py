...
class KoopmanKernelTorch(ABC):
    """Base class for Koopman Kernel Sequencer kernels."""
...
class RBFKernel(KoopmanKernelTorch):
    """RBFKernel for the Koopman Kernel Sequencer."""

    def __init__(self, length_scale: float = 1.0):
        """Initialization.

        Args:
            length_scale (float, optional): _description_. Defaults to 1.0.
        """
...
class KoopKernelSequencerBase(nn.Module):
    """Base class for Koopman Kernel Sequencer models. Not in use yet."""
...
class NystroemKoopKernelSequencer(nn.Module):
    """Nystroem Koopman Kernel Sequencer model."""
    ...
    def __init__(
        ...
        Args:
            ...
            context_mode (str, optional): _description_. Defaults to "full_context".
            mask_koopman_operator (bool, optional): _description_. Defaults to False.
            mask_version (int, optional): _description_. Defaults to 0.
            use_nystroem_context_window (bool, optional): _description_.
                Defaults to False.
        """
        super().__init__()
        self._kernel = kernel
    ...
    #         self.nystrom_data_X = torch.tensor(tcnys_X, dtype=torch.float32).to(device)  # noqa: E501
    #         # shape: (num_nys_centers, input_length, num_feats)
    #         self.nystrom_data_Y = torch.tensor(tcnys_Y, dtype=torch.float32).to(device)  # noqa: E501
    #         # shape: (num_nys_centers, input_length, num_feats)
    ...
    #         # shape: (input_length, num_nys_centers, num_feats),
    #         #     (input_length, num_nys_centers, num_feats)
    ...
    def _initialize_nystrom_data(
        ...
        Args:
            ...
            tensor_context_tgts_train (torch.Tensor): shape
                ...
                (batch_size, n_data // batch_size,
                    input_length, output_length, num_feats)
        """
        ...
        if self.use_nystroem_context_window:
            ...
            if self.output_length == 1:
                ...
                # shape: (num_nys_centers, num_feats)
                # We get the middle entry along the input_length axis,
                # as the edge entries might be not so representative since they
                # correspond sometimes to the start and end positions of the TC.
                self.kernel_nysY_Y = self._kernel(
                    self.nystrom_data_Y, self.nystrom_data_Y
                ) * self.num_nys_centers ** (-1 / 2)
    ...
    def _apply_nystroem_data_X(self, inps):
        ...
        if self.use_nystroem_context_window:
            ...
            # shape: (input_length, num_nys_centers, num_feats),
            #   (input_length, batch_size, num_feats)
    ...
    def _apply_nystroem_data_Y(self, inps):
        if self.context_mode == "full_context":
            ...
            if self.use_nystroem_context_window:
                if self.output_length == 1:
                    # shape self.nystrom_data_Y:
                    #   (num_nys_centers, input_length, num_feats)
                    ...
                    # shape self.nystrom_data_Y:
                    #   (num_nys_centers, input_length, output_length, num_feats)
                ...
                if self.output_length == 1:
                    ...
                    # shape self.nystrom_data_Y:
                    #   (num_nys_centers, output_length, num_feats)
    ...
    def forward(self, inps: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            inps (torch.Tensor): input tensor of shape
                (batch_size, input_length, num_feats)
        ...
        if self.context_mode == "no_context":
            ...
            if self.use_nystroem_context_window:
                raise Exception(
                    " ".join(
                        [
                            "Not implemented. self._apply_nystroem_data_Y() needs",
                            "to be used and adapted to the no_context case.",
                        ]
                    )
                )
            if self.output_length == 1:
...
class KoopKernelLoss(_Loss):
    """Loss function for Koopman Kernel Sequencer training."""

    __constants__ = ["reduction"]  # noqa: RUF012
    ...
    def __init__(
        ...
        Args:
            nystrom_data_Y (torch.Tensor): _description_
            kernel (KoopmanKernelTorch): _description_
            context_mode (str, optional): _description_. Defaults to "no_context".
